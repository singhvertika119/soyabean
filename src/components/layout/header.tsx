...
export default function Header() {
  ...
  return (
    <>
      <header
        className={`border-kawaii-pink sticky top-0 z-50 border-b-2 bg-white/90 backdrop-blur-md dark:border-neutral-800 dark:bg-neutral-900/80 transition-transform duration-300 ease-in-out ${isVisible ? "translate-y-0" : "-translate-y-full sm:translate-y-0"
          }`}
      >
        <Wrapper className="flex items-center justify-between py-2 sm:py-4">
          ...
          <div className="flex items-center gap-4">
            ...
            <nav className="hidden items-center gap-6 sm:flex">
              {headerLinks.map(({ label, href, ...extraProps }) => (
                <AnimatedLink key={href} href={href} {...extraProps}>
                  {label}
                </AnimatedLink>
              ))}
            </nav>
            ...
            >
              <div className="relative">
                <Menu
                  className={`h-5 w-5 text-pink-300 transition-all duration-300 ${isOpen ? "rotate-90 opacity-0" : "rotate-0 opacity-100"
                    }`}
                />
                <X
                  className={`absolute inset-0 h-5 w-5 text-pink-300 transition-all duration-300 ${isOpen ? "rotate-0 opacity-100" : "-rotate-90 opacity-0"
                    }`}
                />
              </div>
      ...
      {/* Mobile Menu Overlay */}
      <div
        className={`fixed inset-0 z-40 transition-all duration-300 sm:hidden ${isOpen ? "pointer-events-auto opacity-100" : "pointer-events-none opacity-0"
          }`}
      >
        {/* Backdrop */}
        ...
        {/* Mobile Menu Panel */}
        <div
          className={`absolute top-0 right-0 h-full w-full transform transition-all duration-300 ease-out ${isOpen ? "translate-x-0" : "translate-x-full"
            }`}
        >
          <div className="border-kawaii-pink h-full border-l-2 bg-gradient-to-br from-white via-pink-50/50 to-white shadow-2xl dark:from-black dark:via-pink-950/20 dark:to-black">
            ...
            <nav className="space-y-2 p-6 mt-15">
              {headerLinks.map(({ label, href, ...extraProps }, index) => (
                <div
                  key={href}
                ...
                >
                  <div onClick={() => setIsOpen(false)}>
                    ...
                    >
                      ...
                      <span className="text-base font-semibold transition-colors">{label}</span>
                    </AnimatedLink>
                  </div>
                </div>
              ))}
